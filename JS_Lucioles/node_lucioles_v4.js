...
async function v0(){
	...
	const mongoUri = 'mongodb+srv://Youssef:Youssef@cluster0.remhf.mongodb.net/lucioles?retryWrites=true&w=majority';	
   ...
   // var mongoUri = process.env.MONGOLAB_URI;
	...
	dbo.listCollections({name: "projecttemperature"})
	    .next(function(err, collinfo) {
		if (collinfo) { // The collection exists
		    //console.log('Collection temp already exists');
		    dbo.collection("projecttemperature").drop() 
		}
	    });

	dbo.listCollections({name: "projectlight"})
	    .next(function(err, collinfo) {
		if (collinfo) { // The collection exists
		    //console.log('Collection temp already exists');
		    dbo.collection("projectlight").drop() 
		}
	    });
